...
Use `opentofu_var_json` for non-secret OpenTofu variables. For a secret variable needed only to build the saved plan, pass its name through `opentofu_plan_secret_var_name` and its value through the `opentofu_plan_secret_var_value` secret; it is exposed as `TF_VAR_<name>` only during the `tofu plan` step. Keep secret values out of `opentofu_plan_args` and `opentofu_plan_secret_args`, which are expanded as command-line arguments.
